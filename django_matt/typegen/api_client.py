...
class APIClientGenerator:
    ...
    def _generate_method(
        ...
        response_type = (
            self._openapi_type_to_typescript(response_schema) if response_schema else "void"
        )
        ...
        lines.append(
            f'    return this.request<{response_type}>("{method.upper()}", path, {options_str});'
        )
        lines.append("  }")
        lines.append("")
        ...
        for controller in controllers:
            ...
            for method_name in dir(controller):
                ...
                if callable(method) and hasattr(method, "_route_info"):
                    ...
                    lines.extend(
                        self._generate_method_from_controller(
                            prefix,
                            method_name,
                            method,
                            route_info,
    ...
    def _generate_method_from_controller(
        ...
        lines.append(
            f'    return this.request<{response_type}>("{http_method}", "{full_path}", {options});'
        )
    ...
    def _generate_react_query_hook(
        ...
        response_schema = json_response.get("schema", {})
        response_type = (
            self._openapi_type_to_typescript(response_schema) if response_schema else "any"
        )
    ...
    def _generate_swr_hook(
        ...
        lines.append(") {")
        lines.append(
            f'  return useSWR<{response_type}>("{method_name}", () => api.{method_name}(), config);'
        )
        lines.append("}")
        lines.append("")
