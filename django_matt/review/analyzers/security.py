...
class SecurityAnalyzer(ASTVisitorAnalyzer):
    ...
    name = "security"
    description = (
        "Checks for secrets, injection, unsafe eval, deserialization, CSRF, debug mode, weak crypto"
    )
    ...
    def _check_weak_crypto(self, node: ast.Call) -> None:
        ...
        is_weak = dotted in self._WEAK_HASH_DOTTED or (
            simple in self._WEAK_HASH_FUNCS and dotted != simple
        )
