...
class ComplexityAnalyzer(ASTVisitorAnalyzer):
    ...
    def _check_function(self, node: ast.FunctionDef | ast.AsyncFunctionDef, *, class_name: str | None) -> None:
        name = node.name
        loc = Location(
    ...
    def _cyclomatic_complexity(self, node: ast.AST) -> int:
        ...
        for child in ast.walk(node):
            if isinstance(child, (ast.If, ast.For, ast.AsyncFor, ast.While, ast.ExceptHandler, ast.With, ast.AsyncWith, ast.Assert)):
                complexity += 1
            elif isinstance(child, ast.BoolOp):
                complexity += len(child.values) - 1
        return complexity

    def _check_cyclomatic(self, node: ast.FunctionDef | ast.AsyncFunctionDef, loc: Location) -> None:
    ...
    def _check_function_length(self, node: ast.FunctionDef | ast.AsyncFunctionDef, loc: Location) -> None:
    ...
    def _max_nesting_depth(self, node: ast.AST) -> int:
        ...
        def _walk(n: ast.AST, depth: int) -> None:
            nonlocal max_depth
            for child in ast.iter_child_nodes(n):
                if isinstance(child, (ast.If, ast.For, ast.AsyncFor, ast.While, ast.With, ast.AsyncWith, ast.Try, ast.TryStar)):
                    new_depth = depth + 1
    ...
    def _check_nesting_depth(self, node: ast.FunctionDef | ast.AsyncFunctionDef, loc: Location) -> None:
    ...
    def _check_parameters(self, node: ast.FunctionDef | ast.AsyncFunctionDef, loc: Location) -> None:
        args = node.args
