...
class ExperimentAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        ...
        ("Strategy", {
            "fields": ["strategy", "epsilon", "exploration_weight"]
        }),
        ("Statistics", {
            "fields": [
                "min_sample_size",
    ...
    def get_queryset(self, request):
        ...
        return qs.annotate(
            _participant_count=Count("assignments")
        )
    ...
    def winner_badge(self, obj):
        if obj.has_winner:
            return format_html(
                '<span style="color: #28a745;">Winner Found</span>'
            )
        return "-"
...
class ExperimentAssignmentAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ["id", "experiment", "variant", "user", "anonymous_id", "assigned_at", "context"]
    raw_id_fields = ["user"]
