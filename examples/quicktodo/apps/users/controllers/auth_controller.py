...
class AuthController(APIController):
    ...
    async def register(request, body: UserCreateSchema) -> dict:
        if await User.objects.filter(email=body.email).aexists():
            raise APIError(message="Email already registered", status_code=400)
        if await User.objects.filter(username=body.username).aexists():
            raise APIError(message="Username already taken", status_code=400)
    ...
    async def login(request, body: LoginSchema) -> dict:
        ...
        if not user.is_active:
            raise APIError(message="Account is disabled", status_code=401)

        tokens = await sync_to_async(create_token_pair)(user)
        return TokenSchema(
    ...
    async def refresh(request, body: dict) -> dict:
        ...
        tokens = await sync_to_async(refresh_tokens)(refresh_token)
        return TokenSchema(
            access_token=tokens.access_token,
            refresh_token=tokens.refresh_token,
        ).model_dump()
    ...
    async def change_password(request, body: ChangePasswordSchema) -> dict:
        ...
        if not check_password(body.current_password, user.password):
            raise APIError(message="Current password is incorrect", status_code=400)
